...
const calculateMoney = async (metres, serPacRate, waiting) => {
  ...
  if (s <= d1) {
    ...
    total = (m1 * serPacRate) + (waiting * WAIT_RATE);
  }
  else if (s <= d2) {
    // ── 1 < s ≤ d2: first‑km flat + (s−d1) at m2
    total = ((m1 + (s - d1) * m2) * serPacRate) + (waiting * WAIT_RATE);
  }
  else if (s <= d3) {
    // ── d2 < s ≤ d3: add the full chunk from d1→d2 at m2, then (s−d2) at m3
    total = ((m1
      + (d2 - d1) * m2
      + (s - d2) * m3) * serPacRate) + (waiting * WAIT_RATE);
  }
  else {
    ...
    total = ( m1
      + (d2 - d1) * m2
      + (d3 - d2) * m3
      + (s - d3) * m4 ) * serPacRate + (waiting * WAIT_RATE);
  }
